...
ALL_ACTION_TOKENS = [P, S, B, PASS, EMPTY]
...
def demonstration_as_dict_creator(data):
    ...
    for time_step in range(len(data)):
        demonstration[time_step] = data[time_step]
...
def find_difference(demonstration_seq):
    ...
    for delta in range(1,len(diff)):
        for x in range(len(diff)):
            for y in range(len(diff[0])):
                if (demonstration_seq[delta][x][y] == demonstration_seq[delta-1][x][y]):
                    continue
                diff[delta] = "Old:" + demonstration_seq[delta-1][x][y] + "New:" + demonstration_seq[delta][x][y]
    return diff
...
interaction = Visualization(dict_seq)
